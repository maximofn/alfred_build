...
alfred_ubuntu=$(docker run -it --rm maximofn/ubuntu_20_04_test1:0.0.1 /usr/src/alfred/alfred.py)
...
alfred_ubuntu=$(docker run -it --rm maximofn/ubuntu_20_04_test2:0.0.1 /usr/src/alfred/alfred.py)
...
alfred_ubuntu=$(docker run -it --rm maximofn/ubuntu_20_04_test3:0.0.1 /usr/src/alfred/alfred.py)
if [[ $alfred_ubuntu == *"You need to set your OpenAI API key in the OPENAI_API_KEY environment variable"* ]]; then
